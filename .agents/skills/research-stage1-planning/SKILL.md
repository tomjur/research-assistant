...
2. **Task-level execution log (planning):** In **`outputs/{research-project-x}/{task-tldr}/`**, call **`resolve_task_execution_log`**, then use **`log_event`** for START blocks and **`log_end_event`** for END blocks (both generate timestamps at write-time; `log_end_event` auto-computes `duration_s`). Log **`planning`** START with fields at least **`planning_phase`:** `stage1_initial` (optional **`trigger`:** e.g. `new_task` / `user_replan`). On **Stage I replan** of an existing mission, log another **START**/**END** pair with **`planning_phase`:** `stage1_replan` before and after that replan pass. **END** **planning** only when the [Output checklist](#output-checklist) artifacts for that pass are satisfied, with **END** fields including **`todo_rows`**, **`task_graph_nodes`**, **`waves_computed`:** `yes` or `no`. API names: [Helper APIs](../../research%20projects/docs/SKILLS_AND_SCRIPTS.md#helper-apis). **`PYTHONPATH`:** [HOST_TOOLS — Python](../../research%20projects/docs/HOST_TOOLS.md#python).
   - **Step events during planning:** Use **`log_step`** (or manual `step` blocks) for discrete actions within this phase:
     - `web_search` — each search-term validation query, with `query` and optional `results_summary`.
     - `script_call` — when calling `task_graph_waves` or other helpers, with `script` and optional `result`.
     - `skill_invocation` — when opening `research-task-graph-waves` or other skills, with `skill`.
     - `user_decision` — when the user resolves an ambiguous term, scope question, or deliverable constraint, with `question`, `decision`, optional `notes`.
