...
| `$WorkersPerTask$` | **Total cap** of delegated worker runs per **(prompt variant, worker model)** pair. Early stopping (see `$EarlyStoppingWorkers$`) may finish a pair with fewer. Log **intended** vs **completed** counts. |
| `$EarlyStoppingWorkers$` | Per-vendor **batch size per round** AND **consecutive-no-new-info threshold** for stopping that vendor within a prompt variant. Default: **2**. Each round, vendor V launches `max(0, $EarlyStoppingWorkers$ − consecutive_no_new_V)` workers (capped by remaining `$WorkersPerTask$` budget). After extracting atomic findings from each validated output, a worker that adds 0 new findings increments the counter; any new finding resets it. When the counter reaches `$EarlyStoppingWorkers$`, vendor V stops for this prompt variant. Setting this equal to `$WorkersPerTask$` disables early stopping (one big batch). See [research-extract-atomic-findings](../.agents/skills/research-extract-atomic-findings/SKILL.md) and [research-compare-atomic-findings](../.agents/skills/research-compare-atomic-findings/SKILL.md). |
...
3. For each **(prompt variant, worker model)** pair, launch up to **`$WorkersPerTask$`** delegated workers in **rounds** of **`$EarlyStoppingWorkers$`** per vendor (each labeled with **copy number**). Each round, vendor V launches `max(0, $EarlyStoppingWorkers$ − consecutive_no_new_V)` workers (capped by remaining budget), in **parallel** across distinct slots when the host supports it. Each **worker** child must be launched with that slot’s **worker** model (from **`$ModelsForThisTask$`**). After every validator in a round returns, extract atomic findings from each validated output ([research-extract-atomic-findings](../.agents/skills/research-extract-atomic-findings/SKILL.md)) and compare against that vendor's accumulated set ([research-compare-atomic-findings](../.agents/skills/research-compare-atomic-findings/SKILL.md), called twice with swapped arguments). A validated output that contributes zero new atomic findings increments that vendor's `consecutive_no_new` counter; any new finding resets it. Vendor V stops for this prompt variant when its counter reaches **`$EarlyStoppingWorkers$`**. Operational detail: [research-orchestrator/SKILL.md](../.agents/skills/research-orchestrator/SKILL.md).
...
5. The orchestrator **compares** validated outputs using the **Merge procedure** ([research-orchestrator/SKILL.md](../.agents/skills/research-orchestrator/SKILL.md)): two extractions + two directional compares, partitioned into `only_A` / `only_B` / `common`. This same procedure drives (i) same-vendor early stopping within a round (step 3), (ii) cross-vendor merge within a prompt variant, and (iii) cross-prompt-variant convergence here. If more effort is warranted and repeats are dominating, collect **additional unique** prompt variants by **re-running step 1** (do not restate variant-stop rules here). When **`$MaxOrchestratorIterations$`** is set, do not start more than that many **refinement rounds** after the initial worker wave (each round may add variants and fan out workers again).
