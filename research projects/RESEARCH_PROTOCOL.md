...
- Contains this protocol, **shared skills** (`skills/`), **shared scripts** (`scripts/`), **templates** (`templates/`), and one folder per research project.

**`{research-project-x}`** — one folder per broad research program.

- Entry point for all questions that belong to the same overall topic.
- Agents **may** read final reports from **earlier tasks under this same project**.
- Agents **must not** reference data from **other** `{research-project-x}` directories.

**`{task-tldr}`** — one folder per distinct user query / mission.
...
- **Execution log:** One append-only **`log_{datetime}.txt`** per mission (filesystem-safe datetime); **START**/**END** per worker and validator across all `{sub-task}/` folders. **Include:** task graph **id**, subtask folder name, child **prompt**, **artifact paths**, **status** (success / failed / cancelled; one-line **reason** if failed), **prompt hash**, **worker model**, **copy**, **`$ControlPlaneModel$`** on validators, optional host run id. **Exclude** response bodies. **Pointer:** **`execution_log_active.txt`**. Prefer **`research_utils.execution_log`** ([Python helpers](#python-helpers)).

**`{sub-task}`** — one folder per **orchestrator** line item from the TODO.
...
**`workers/` and `validators/`** under each `{sub-task}`:
...
8. **Deliverable length:** If the user did **not** specify report length or depth in the original prompt, **ask** and record the answer under **`USER_PROMPT.md`** in **Deliverable constraints** (target words/pages or qualitative depth). Alternatively, record an explicit **waiver** (e.g. "use protocol default depth") so Stage II knows the constraint was considered. Stage III **`FINAL_RESPONSE.md`** must **honor** what is recorded there (or the agreed default).
...
11. Do **not** start orchestrator delegation until **`TOOLS_AND_MCP.md`** lists **confirmed** **`$ControlPlaneModel$`** and at least one **`$ModelsForThisTask$`** entry, MCP/tools needed for the mission (or documented gaps with user approval), and **`USER_PROMPT.md`** includes **Approved search terms** and **Deliverable constraints** (report length/depth **or** explicit waiver). Only after planning is stable, create `{sub-task}` folders and proceed to Stage II. Task templates link here instead of restating this list.
...
- Log **intended** vs **completed** workers in `orchestrator_log.md` and optional `orchestrator_meta.json`. Task-level **`log_{datetime}.txt`**: [**`{task-tldr}`**](#filesystem-structure); prefer **`research_utils.execution_log`** ([Python helpers](#python-helpers)).
...
2. Write **`FINAL_RESPONSE.md`** in `{task-tldr}/`.
