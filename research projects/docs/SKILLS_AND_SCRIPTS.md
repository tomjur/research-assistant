...
- **`execution_log`:** `resolve_task_execution_log`, `render_execution_record`, `append_execution_record`, `format_log_filename`, `CANONICAL_LOG_PATTERN`, `utc_timestamp_iso`, `ExecutionLogRole`, **`log_event`**, **`log_end_event`**, **`log_step`**, **`parse_duration_from_start`** — task-level **`log_YYYY-MM-DD_HHMMSS.txt`** (UTC) in **`outputs/{research-project-x}/{task-tldr}/`**. **Preferred runtime API:** **`log_event(log_path, phase, role, fields)`** generates the timestamp at write-time (cannot be batched); **`log_end_event(log_path, role, fields, match_fields)`** also auto-computes `duration_s` from the matching START block. Use `render_execution_record` + `append_execution_record` only when you need a **pinned** timestamp (e.g. tests). **`render_execution_record`** roles: `worker`, `validator`, `planning`, `subtask`, `host_subagent`, **`step`**. **`log_step(log_path, parent_role, action, fields)`** appends a single `step` block (no START/END pair) for script calls, skill invocations, dedup stats, user decisions, and web searches. **`parse_duration_from_start(log_path, role, match_fields)`** scans backwards for the matching START block and returns wall-clock seconds to now (for `duration_s` on END blocks). — [RESEARCH_PROTOCOL — Filesystem structure](../RESEARCH_PROTOCOL.md#filesystem-structure); [orchestrator `scripts/`](../../.agents/skills/research-orchestrator/scripts/).
...
| Orchestrator inline: extract atomic findings from a text | [research-extract-atomic-findings/SKILL.md](../../.agents/skills/research-extract-atomic-findings/SKILL.md) |
| Orchestrator inline: directional compare of two finding sets (call twice, swapped) | [research-compare-atomic-findings/SKILL.md](../../.agents/skills/research-compare-atomic-findings/SKILL.md) |
